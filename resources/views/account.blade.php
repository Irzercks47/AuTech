           ...
           <div class="col-2">
              <div class="form-container">
                ...
                @if(session()->has('loginError'))
                    <p class="danger">{{session('loginError')}}</p>
                @endif           
                <form id="LoginForm" action="/account" method="post">
                    @csrf
                    <input type="email" placeholder="Email" name="email" id="email" required autofocus class="@error('email') is-invalid @enderror" value="{{old('email')}}">
                        @error('email')
                            <small class="inval-feed">{{$message}}</small>
                        @enderror
                    <input type="password" placeholder="Password" name="password" id="password" required>
                    <button type="submit" class="btn">Login</button>
                    <small>Dont have account, <a href="/register"> register here </a>!!</small>
